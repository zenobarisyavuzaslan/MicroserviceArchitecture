...
namespace FreeCourse.Services.Discount.Services
{
    public class DiscountService : IDiscountService
    {
        ...
        {
            var DeletedStatus = await _dbConnection.ExecuteAsync("DELETE from discount where id=@Id", new { Id = id });
            return DeletedStatus > 0 ? Response<NoContent>.Success(204) : Response<NoContent>.Fail("Discount not found", 404);
        }
        ...
        {
            var discounts = await _dbConnection.QueryAsync<Models.Discount>("select * from discount where code=@Code and userid=@UserId", 
